...
from model.tsm.module import Conv2d, BatchNorm2d, Relu, MaxPool2d, Sequential
...
class BasicBlock(nn.Module):
    ...
    def forward(self, x, flows):
        ...
        out = self.conv1(x, flows)
        out = self.bn1(out, flows)
        out = self.relu(out, flows)

        out = self.conv2(out, flows)
        out = self.bn2(out, flows)

        if self.downsample is not None:
            identity = self.downsample(x, flows)
...
class Bottleneck(nn.Module):
    ...
    def forward(self, x, flows):
        ...
        out = self.conv3(out, flows)
        out = self.bn3(out, flows)
...
class ResNet(nn.Module):
    ...
    def _forward_impl(self, x, flows):
        # See note [TorchScript super()]
        x = self.conv1(x, flows)
        x = self.bn1(x, flows)
        x = self.relu(x, flows)
        x = self.maxpool(x, flows)

        x_1 = self.layer1(x, flows)
        x_2 = self.layer2(x_1, flows)
        x_3 = self.layer3(x_2, flows)
        x_4 = self.layer4(x_3, flows)

        return [x_1, x_2, x_3, x_4]

    def forward(self, x, flows):
        return self._forward_impl(x, flows)
